...
const MonitoringDashboard: React.FC<MonitoringDashboardProps> = (props) => {
    ...
    const isTrechoFilter = selectedService !== 'ALL' && trechoServices.includes(selectedService);
    const locFilterLabel = isTrechoFilter ? 'Trecho' : (isCorteFilter ? 'Corte' : (selectedService === 'ALL' ? 'Localização' : 'Obra de Arte (OAE)'));
            ...
            <main className="flex-1 overflow-y-auto relative custom-scrollbar">
                <div className="p-8 pb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 p-5 bg-[#0a0f18]/80 backdrop-blur-3xl border border-white/5 rounded-3xl mb-8 shadow-2xl">
                        ...
                        <div className="flex flex-col gap-1.5"><label className="text-[10px] font-black text-gray-500 uppercase flex items-center gap-1"><Target size={10} /> {locFilterLabel}</label><select value={selectedOAE} onChange={(e) => setSelectedOAE(e.target.value)} className="bg-black/40 border border-white/10 rounded-xl p-2.5 text-xs font-bold text-white outline-none focus:border-brand-accent transition-all">{localizacoes.map(o => <option key={o} value={o}>{o === 'ALL' ? (isTrechoFilter ? 'Todos os Trechos' : (isCorteFilter ? 'Todos os Cortes' : 'Todas as Localizações')) : o}</option>)}</select></div>
