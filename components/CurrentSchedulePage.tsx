    ...
    const DisplayView = () => (
        <div className="bg-brand-dark/70 p-4 rounded-lg animate-fade-in space-y-4">
            <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
                ...
                {currentScheduleTasks.length > 0 && (
                    <button
                        onClick={() => exportTasksToExcel(currentScheduleTasks, `Cronograma_Corrente_${new Date().toISOString().split('T')[0]}`)}
                ...
                <table className="w-full table-auto divide-y divide-brand-darkest text-[10px] leading-tight">
                    <thead className="bg-brand-darkest/80">
                        <tr>
                            ...
                            <th className="px-2 py-2 text-left font-medium text-brand-med-gray uppercase tracking-wider">Lado</th>
                    ...
                    <tbody className="divide-y divide-brand-darkest">
                        {currentScheduleTasks.map(task => (
                            <tr key={task.id} className="hover:bg-brand-dark/50 transition-colors">
                                ...
                                <td className="px-2 py-1.5 align-middle text-gray-400 break-words max-w-[60px]">{task.side || '-'}</td>
                                <td className="px-2 py-1.5 align-middle text-gray-400 break-words max-w-[60px]">{task.corte || '-'}</td>
                                <td className="px-2 py-1.5 align-middle text-gray-300 text-right font-mono">{formatNumberBR(task.quantity, 2)}</td>
