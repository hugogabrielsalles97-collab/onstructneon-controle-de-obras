...
const TaskListView: React.FC<TaskListViewProps> = ({ tasks, baselineTasks, onEditTask, onDeleteTask, onSort, sortConfig, userRole, allUsers }) => {
  ...
  // Busca leve: quais tarefas TEM fotos (só IDs, sem carregar imagens)
  useEffect(() => {
    fetchTaskIdsWithPhotos().then(ids => setTaskIdsWithPhotos(ids));
  }, [tasks]); // re-busca quando a lista de tasks muda (ex: após salvar nova foto)
      ...
      <table className="min-w-full border-separate border-spacing-y-2 px-4 table-print">
        <thead>
          <tr className="bg-transparent">
            ...
            <th className="px-4 py-4 text-right text-[10px] font-black text-brand-med-gray uppercase tracking-widest non-printable sticky right-0 z-20 bg-[#0a101a]">Ações</th>
          </tr>
        </thead>
        <tbody>
          {(() => {
            ...
            return tasks.map((task, index) => {
              ...
              return (
                ...
                >
                  ...
                  <td className="px-5 py-5 rounded-r-2xl border-r border-t border-b border-white/5 align-middle text-right non-printable sticky right-0 z-10 bg-[#0d131f] group-hover:bg-[#101624]">
