...
function testParse() {
  ...
  sheets.forEach(sheetName => {
    ...
    for (let r = 0; r < 15; r++) {
      ...
      if (row.some(c => String(c || '').toUpperCase().includes('OAE'))) {
        ...
        row.forEach((cell, idx) => {
          ...
          if (val === 'OAE / APOIO') { colMap.oae = idx; colMap.apoio = -idx; } 
    ...
    console.log(`Sheet: ${sheetName} | HeaderIdx: ${headerRowIdx} | ColMap:`, colMap);
    ...
    if (headerRowIdx !== -1) {
       for (let r = headerRowIdx + 1; r < headerRowIdx + 5; r++) {
          ...
          if (isStatusRow) {
             console.log(`  Row ${r}: Resp=${row[colMap.resp]}, OAE=${row[colMap.oae]}, Status=${statusVal}`);
          }
       }
    }
  });
